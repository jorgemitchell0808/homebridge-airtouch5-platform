...
exports.Airtouch5Wrapper = void 0;
const api_1 = require("./api");
const platformZoneAccessory_1 = require("./platformZoneAccessory");
const platformACAccessory_1 = require("./platformACAccessory");
...
class Airtouch5Wrapper {
    constructor(ip, consoleId, AirtouchId, deviceName, log, emitter, platform) {
        ...
        this.api = new api_1.AirtouchAPI(ip, consoleId, AirtouchId, deviceName, log, emitter);
        this.api.connect();
     ...
     */
    initialiseZonesForAc(ac_number, ac_ability) {
        const start_zone = +ac_ability.ac_start_zone;
        const count_zones = +ac_ability.ac_zone_count;
        // Create zones for the range [start_zone, start_zone + count_zones)
        for (let i = start_zone; i < (start_zone + count_zones); i++) {
     ...
     */
    AddUpdateZoneStatus(zone_status) {
     ...
     */
    createZone(zone_number, ac_number) {
     ...
     * @param ac_number - Parent AC unit number
     */
    registerZone(zone_number, ac_number) {
        ...
        let platform_accessory = this.platform.findAccessory(this.AirtouchId, ac_number, magic_1.MAGIC.ZONE_OR_AC.ZONE, zone_number);
        if (platform_accessory === undefined) {
            ...
            platform_accessory.context.zone_or_ac = magic_1.MAGIC.ZONE_OR_AC.ZONE;
            // Register with HomeKit
            this.platform.api.registerPlatformAccessories(settings_1.PLUGIN_NAME, settings_1.PLATFORM_NAME, [platform_accessory]);
        }
        // Create our zone accessory wrapper
        const zone_accessory = new platformZoneAccessory_1.AirTouchZoneAccessory(this.platform, platform_accessory, this.AirtouchId, zone_number, this.zones[zone_number], this.acs[ac_number], this.log, this.api);
    ...
    registerAc(ac_number) {
        ...
        // Find existing accessory or create new one
        let platform_accessory = this.platform.findAccessory(this.AirtouchId, ac_number, magic_1.MAGIC.ZONE_OR_AC.AC);
        if (platform_accessory === undefined) {
            // Create new platform accessory
            platform_accessory = new this.platform.api.platformAccessory(this.acs[ac_number].ac_ability.ac_name, uuid);
            platform_accessory.context.ac_number = ac_number;
            platform_accessory.context.AirtouchId = this.AirtouchId;
            platform_accessory.context.zone_or_ac = magic_1.MAGIC.ZONE_OR_AC.AC;
            // Register with HomeKit
            this.platform.api.registerPlatformAccessories(settings_1.PLUGIN_NAME, settings_1.PLATFORM_NAME, [platform_accessory]);
        }
        // Create our AC accessory wrapper
        const ac_accessory = new platformACAccessory_1.AirTouchACAccessory(this.platform, platform_accessory, this.AirtouchId, ac_number, this.acs[ac_number], this.zones, this.log, this.api);
        // Link the accessory and mark as registered
     ...
     */
    getAvailableFanSpeeds(ac_ability) {
        const speeds = [];
     ...
     */
    getZoneControlType(zone_status) {
     ...
     */
    getZoneSensorStatus(zone_status) {
     ...
     */
    getZoneBatteryStatus(zone_status) {
     ...
     */
    getZoneSpillStatus(zone_status) {
...
exports.Airtouch5Wrapper = Airtouch5Wrapper;
//# sourceMappingURL=airTouchWrapper.js.map